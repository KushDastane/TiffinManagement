...
const UPI_ID = "9820060064@ybl"; // Ideally from config
  ...
  return (
    <View style={tw`flex-1 bg-[#faf9f6]`}>
      ...
      <ScrollView contentContainerStyle={tw`p-6 pt-44 pb-20`} showsVerticalScrollIndicator={false}>
        ...
        {isAmountValid && (
          <View>
            {/* 2. Mode select - Minimalist Pills */}
              ...
              {["UPI", "CASH"].map(mode => (
                <Pressable
                  ...
                  style={[
                    ...
                    paymentMode === mode
                      ? tw`bg-gray-900 border-gray-900 shadow-sm shadow-gray-400/20`
                      : tw`bg-white border-gray-100`
                  ]}
                >
                  {mode === "UPI" ? (
                    <Smartphone size={14} color={paymentMode === mode ? "white" : "#9ca3af"} />
                  ) : (
                    <CreditCard size={14} color={paymentMode === mode ? "white" : "#9ca3af"} />
                  )}
                  <Text style={[tw`font-black text-[10px] uppercase tracking-widest`, paymentMode === mode ? tw`text-white` : tw`text-gray-400`]}>{mode}</Text>
                </Pressable>
              ))}
            </View>
            ...
            {paymentMode === "UPI" ? (
              <View>
                {/* Pay Button - Premium Minimalist */}
                <Pressable
                  onPress={handleUPIPay}
                  style={tw`bg-white rounded-2xl p-4 shadow-sm border border-gray-100 mb-6 flex-row items-center justify-between`}
                >
                  <View style={tw`flex-row items-center gap-3`}>
                    <View style={tw`bg-emerald-50 w-10 h-10 rounded-xl items-center justify-center`}>
                      <Smartphone size={20} color="#059669" />
                    </View>
                    <View>
                      <Text style={tw`text-sm font-black text-gray-900`}>Pay via UPI App</Text>
                      <Text style={tw`text-[9px] font-bold text-gray-400 uppercase tracking-tighter`}>Instant Transfer</Text>
                    </View>
                  </View>
                  <View style={tw`bg-emerald-50 p-2 rounded-full`}>
                    <ArrowLeft size={16} color="#059669" style={{ transform: [{ rotate: '180deg' }] }} />
                  </View>
                </Pressable>
              ...
              <View style={tw`bg-white rounded-2xl p-7 border border-gray-100 mb-6 items-center`}>
                <View style={tw`bg-yellow-50 w-14 h-14 rounded-2xl items-center justify-center mb-4`}>
                  <CreditCard size={28} color="#ca8a04" />
                </View>
                <Text style={tw`text-base font-black text-gray-900`}>Cash Payment</Text>
                <Text style={tw`text-xs text-gray-500 text-center mt-2 leading-4`}>
            ...
            <View style={tw`mt-4`}>
              ...
              >
                {loading ? <ActivityIndicator color="white" /> : (
                  <Text style={[tw`font-black text-[11px] uppercase tracking-widest`, isFormValid ? tw`text-white` : tw`text-gray-400`]}>
                    Confirm & Add Money
                  </Text>
                )}
              </Pressable>
